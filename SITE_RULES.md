...
* `cpp.html`（含資料夾/accordion 計數器）顯示的文章數：

  * 必須由 `site-index.json` 計算取得
  * 禁止硬編碼固定數字
  * scope=cpp-notes 時，僅計入 `cpp-notes/all problems/`（沿用第 6.5）
...
### 10.9（法律）新增文章後必須觸發第 6 節索引更新流程（必遵守）
...
### 11.4 categories.html「相關文章」命中規則（必遵守）
